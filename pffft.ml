(* type res = | Yield of (unit -> res) | Fork of (unit -> unit) * (unit -> res)
   | Done of bool *)

(* let pt = Delimcc.new_prompt () *)

let pt2 = Delimcc.new_prompt ()

(* let yield () = Delimcc.shift pt (fun g -> Yield g)

   let fork f = Delimcc.shift pt (fun g -> Fork (f, g))

   let exit b = Delimcc.shift pt (fun _ -> Done b) *)
...
let forall_bool () =
  print_endline "forall_bool -> true";
  Delimcc.shift pt2 (fun cont ->
      (* Delimcc.shift pt2 (fun cont -> cont true); *)
      cont false)
...
(* less-efficient abort *)

let shouldbe msg fmt vexp v =
  if v = vexp then printf "%s gave the expected answer %s\n" msg (fmt v)
  else (
...
let test3' =
  let () = printf "test3'\n" in
  let p = new_prompt () in
  let v = push_prompt p (fun () -> push_prompt p (fun () -> abort p 5 + 6)) in
  shouldbe "test3'" int 9 (v + 4)
...
(* The classical shift test (display (+ 10 (reset (+ 2 (shift k (+ 100 (k (k
   3)))))))) ; --> 117 *)
...
let test5 =
  ...
  let v0 =
    push_prompt p0 (fun () ->
        let v = shift p0 (fun sk -> sk (sk 3) + 100) in
        v + 2)
  in
...
let test5' =
  ...
  let v0 =
    push_prompt p0 (fun () ->
        let v = shift p0 (fun sk -> sk 3 + 100) in
        v + 2)
  in
  ...
  let v0 =
    push_prompt p0 (fun () ->
        let v =
          shift p0 (fun sk ->
              sk (fun () ->
                  push_prompt p1 (fun () -> 9 + sk (fun () -> abort p1 3)))
              + 100)
        in
        v () + 2)
  in
  ...
  let v0 =
    push_prompt p0 (fun () ->
        let v =
          shift p0 (fun sk ->
              sk (fun () ->
                  push_prompt p1 (fun () -> 9 + sk (fun () -> abort p0 3)))
              + 100)
        in
        v () + 2)
  in
...
let test6 =
  ...
  let pushtwice sk =
    push_subcont sk (fun () -> push_subcont sk (fun () -> 3))
  in
  let v0 =
    push_prompt p1 (fun () ->
        push_prompt p2 (fun () -> take_subcont p1 (fun sk () -> pushtwice sk))
        + 1)
  in
    ...
    push_subcont sk (fun () ->
        push_subcont sk (fun () ->
            take_subcont p2 (fun sk2 () ->
                push_subcont sk2 (fun () -> push_subcont sk2 (fun () -> 3)))))
  in
  let v =
    push_prompt p1 (fun () ->
        push_prompt p2 (fun () ->
            push_prompt p3 (fun () ->
                take_subcont p1 (fun sk () -> pushtwice sk))
            + 10)
        + 1)
  in
...
let test7' =
  ...
  let pushtwice sk =
    sk (fun () ->
        sk (fun () ->
            shift p2 (fun sk2 -> sk2 (fun () -> sk2 (fun () -> 3))) ()))
  in
  let v =
    push_prompt p1 (fun () ->
        push_prompt p2 (fun () ->
            push_prompt p3 (fun () -> shift p1 pushtwice ()) + 10)
        + 1)
  in
  shouldbe "test7'" int 135 (v + 100)
...
let test9 =
  ...
  try
    push_prompt p (fun () ->
        try shift p (fun sk -> raise E1) with E1 -> assert false)
  with E1 -> printf "OK\n"
...
let test9' =
  ...
  try
    push_prompt p (fun () ->
        try shift p (fun sk -> sk (sk (raise E1))) with E1 -> assert false)
  with E1 ->
    printf "Cleared to top. Now check that prompts are cleared\n";
    assert (not (is_prompt_set p))
...
let test9''' =
  ...
  shouldbe "test9'''" int 21
    (push_prompt p (fun () ->
         ...
         try
           push_prompt p (fun () ->
               debug_status "in test9'''";
               raise E1 + 800)
         with E1 -> shift p (fun sk -> sk (sk 1))))

(* (reset (let ((x (shift f (cons 'a (f '()))))) (shift g x)))) ==> '(a) *)
...
let test_t1 =
  ...
  List.iter (fun e -> printf " %d" e) res;
  printf "\n"
