...
(* let values = Flux.unfold (fun cpt -> if cpt < 50 then Some (cpt, cpt + 1)
   else None) 2 in Pffft.check Pffft.( fun () -> let a = forall values in let b
   = forsome values in assumption (fun () -> a <> b); let r = premier a in
